    ...
    <footer className="site-footer">
      <div className="shell footer-grid">
        <div className="footer-brand">
          ...
          <p className="muted">Web design. Brand identity.<br />Social media management.</p>
        </div>
        ...
        <div><h3>Follow</h3>{socialLinks.map((link) => <a key={link.label} href={link.href} aria-label={`${link.label} placeholder link`}>{link.label}</a>)}</div>
      </div>
