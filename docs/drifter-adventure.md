...
- The broken gravity device now produces a gentle continuous bob, roughly one cycle every 2.5–3.9 seconds depending on the planet. A damped spring replaces repeated floor-impact jumps. Hold BOOST to rise toward higher platforms; the spring potion increases the bob. Ordinary terrain rises and short seams remain traversable, while wide voids still cause falls. Gravity locks, swimming, flight and vehicles retain their distinct movement.
...
`tests/drifter-gravity.test.mjs` checks the idle cycle and smooth velocity changes, boost and potion effects, short seams versus wide gaps, platform underside rejection, unboosted traversal and frame-rate consistency. The full expedition tests also cover pickup placement around platform edges. `scripts/check-drifter-gear.cjs` checks easy gear access, pause/resume, equipment swaps and phone control layouts in a real browser.
