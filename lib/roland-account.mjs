...
import {createInsightsService} from './drifter-insights.mjs';
...
/** @param {{databasePath?: string, databaseUrl?: string, databaseDriver?: string, origin?: string, origins?: string[], local?: boolean, adminEmail?: string, bootstrapToken?: string}} options */
export async function createGameService({databasePath,databaseUrl,databaseDriver,origin,origins=[],local=false,adminEmail='',bootstrapToken=''}={}){
 if(!origin||(!databaseUrl&&(!databasePath||databaseDriver==='pg'||process.env.VERCEL&&!local)))return{handle:async request=>new URL(request.url).pathname.endsWith('/status')?unavailable():json({error:'Account saves are not connected on this server.'},503),close:()=>{}};
 const db=await openGameDatabase({databasePath,databaseUrl,databaseDriver}),storage=databaseDriver==='pg'?'server':databaseUrl?'cloud':local?'local':'server';
 let hashing=0;
 ...
 const insights=await createInsightsService({db,json,body,rate,load,adminEmail,bootstrapToken});
 async function handle(request,{clientAddress='local'}={}){try{
  ...
  const insightResponse=await insights.handle(action,request,user,clientAddress);if(insightResponse)return insightResponse;
  ...
  if(action==='save'){
   ...
   if(method==='PUT'){
    ...
    await insights.refreshProfile(user.id,merged).catch(()=>{});
